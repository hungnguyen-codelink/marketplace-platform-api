...
exports.shopsService = exports.ShopsService = void 0;
const client_1 = require("../../db/client");
const errors_1 = require("../../errors");
class ShopsService {
    async createShop(sellerId, payload) {
        const { name, description, banner_url, contact_email } = payload;
        try {
            const result = await client_1.db.query('INSERT INTO shops (seller_id, name, description, banner_url, contact_email) VALUES ($1, $2, $3, $4, $5) RETURNING id, seller_id, name, description, banner_url, contact_email, created_at', [sellerId, name, description || null, banner_url || null, contact_email || null]);
            ...
            if (error.code === '23505' && error.constraint === 'shops_seller_id_key') {
                throw new errors_1.ConflictError('Seller already has a shop');
            }
    ...
    async getShopBySellerId(sellerId) {
        const result = await client_1.db.query('SELECT id, seller_id, name, description, banner_url, contact_email, created_at FROM shops WHERE seller_id = $1', [sellerId]);
        if (result.rows.length === 0) {
            throw new errors_1.NotFoundError('Shop not found');
        }
        const shop = result.rows[0];
        return {
            id: shop.id,
    ...
    async getShopById(shopId) {
        const result = await client_1.db.query('SELECT id, name, description, banner_url, contact_email, created_at FROM shops WHERE id = $1', [shopId]);
        if (result.rows.length === 0) {
            throw new errors_1.NotFoundError('Shop not found');
        }
        const shop = result.rows[0];
    ...
    async updateShop(sellerId, shopId, payload) {
        // First verify the shop exists and belongs to this seller
        const shopResult = await client_1.db.query('SELECT seller_id FROM shops WHERE id = $1', [shopId]);
        ...
        if (fields.length === 0) {
            ...
            const result = await client_1.db.query('SELECT id, seller_id, name, description, banner_url, contact_email, created_at FROM shops WHERE id = $1', [shopId]);
